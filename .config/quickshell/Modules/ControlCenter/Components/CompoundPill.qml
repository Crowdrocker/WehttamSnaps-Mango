...
Rectangle {
    ...
    width:  parent.width
    implicitHeight: isVolumeMixerWidget ? column.implicitHeight + (Theme.spacingXS + 1) + Theme.spacingS
        : (isAudioWidget
            ? Math.max(100, column.implicitHeight + (Theme.spacingXS + 1) + Theme.spacingS)
            : 56)
    ...
    clip:     true
        ...
        Column {
            ...
            Loader {
                id: audioEmbedLoader
                width:  parent.width
                // Device chooser renders in WidgetGrid's full-width DetailHost.
                // Keep inline embed only when CompoundPill is used standalone.
