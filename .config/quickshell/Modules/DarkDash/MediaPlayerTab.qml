...
Item {
    ...
    function isVolatileArtworkSource(src) {
        const s = String(src || "")
        if (!s) return true
        return s.includes("/tmp/.com.microsoft.Edge.")
            || s.includes("/tmp/.org.chromium.Chromium.")
            || s.includes("/tmp/.com.google.Chrome.")
            || s.includes("/var/tmp/.com.microsoft.Edge.")
            || s.includes("/var/tmp/.org.chromium.Chromium.")
            || s.includes("/var/tmp/.com.google.Chrome.")
    }
    ...
    function findNewestCiderCacheFile(dirPath) {
        try {
            console.log("[MediaPlayer] findNewestCiderCacheFile: Looking in", dirPath)
            const process = Quickshell.Processes.spawn(["sh", "-c", "ls -t " + dirPath + "/.org.chromium.Chromium.* 2>/dev/null | head -1"], {stdout: "pipe"})
            const output = process.readAll().toString().trim()
            console.log("[MediaPlayer] findNewestCiderCacheFile: Found:", output)
            return output || ""
        } catch (e) {
            console.log("[MediaPlayer] findNewestCiderCacheFile: Error:", e)
            return ""
        }
    }

    function loadArtwork(url) {
        console.log("[MediaPlayer] loadArtwork called with URL:", url)
        if (!url) {
            console.log("[MediaPlayer] loadArtwork: No URL provided")
            return
        }

        if (url.startsWith("http://") || url.startsWith("https://")) {
            console.log("[MediaPlayer] loadArtwork: HTTP/HTTPS URL detected")
            ...
            if (url.startsWith("file://")) {
                var path = url.substring(7) // Remove "file://" prefix
                console.log("[MediaPlayer] loadArtwork: file:// URL, extracted path:", path)
                if (path.includes(".org.chromium.") ||
                    path.includes(".com.microsoft.Edge.") ||
                    path.includes(".com.google.Chrome.") ||
                    path.includes("/.var/app/") ||
                    path.includes(".var/app/sh.cider.Cider/cache/") ||
                    path.startsWith("/tmp/") ||
                    path.startsWith("/var/tmp/")) {
                    // For Cider Chromium cache files, find the newest one
                    if (path.includes(".var/app/sh.cider.Cider/cache/tmp/") && path.includes(".org.chromium.Chromium.")) {
                        console.log("[MediaPlayer] loadArtwork: Cider Chromium cache detected, finding newest file")
                        const dirPath = path.substring(0, path.lastIndexOf("/"))
                        const newestFile = findNewestCiderCacheFile(dirPath)
                        if (newestFile) {
                            console.log("[MediaPlayer] loadArtwork: Loading newest cache file:", newestFile)
                            _preloadImage.source = newestFile
                            return
                        }
                    }
                    // Allow browser cached URLs and Flatpak app data - use path without file:// prefix
                    console.log("[MediaPlayer] loadArtwork: Loading file path:", path)
                    _preloadImage.source = path
                    return
                }
            }
            console.log("[MediaPlayer] loadArtwork: Loading as URL:", url)
    ...
    Image {
        ...
        onStatusChanged: {
            if (status === Image.Ready) {
                if (root.isVolatileArtworkSource(source)) {
                    _cqSource = ""
                    colorQuantizer.source = ""
                } else {
                    _cqSource = source
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                Row {
                                    ...
                                    EHIcon {
                                        name: modelData ? getAudioDeviceIcon(modelData) : "speaker"
                                        size: 20
            ...
            Item {
                ...
                Item {
                    ...
                    Rectangle {
                        id: sliderFill
                        width: parent.width
                        height: defaultSink?.audio ? (Math.min(1.0, defaultSink.audio.volume) * parent.height) : 0
                        anchors.bottom: parent.bottom
                        anchors.horizontalCenter: parent.horizontalCenter
                        color: Theme.primary
                    ...
                    Rectangle {
                        ...
                        radius: Theme.cornerRadius
                        y: {
                            const ratio = defaultSink?.audio ? Math.min(1.0, defaultSink.audio.volume) : 0
