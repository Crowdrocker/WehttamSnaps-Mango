...
Rectangle {
    ...
    property date selectedDate: (systemClock && systemClock.date) ? systemClock.date : new Date()
    ...
    function updateSelectedDateEvents() {
        if (CalendarService && CalendarService.khalAvailable) {
            const events = selectedDate && !isNaN(selectedDate.getTime()) && CalendarService ? CalendarService.getEventsForDate(selectedDate) : []
    ...
    Column {
        ...
        Item {
            ...
            StyledText {
                ...
                height: 40
                anchors.verticalCenter: parent.verticalCenter
                text: selectedDate && !isNaN(selectedDate.getTime()) ? (hasEvents ? (Qt.formatDate(selectedDate, "MMM d") + " • " + (selectedDateEvents.length === 1 ? "1 event" : selectedDateEvents.length + " events")) : Qt.formatDate(selectedDate, "MMM d")) : ""
        ...
        Grid {
            ...
            property date displayDate: systemClock ? systemClock.date : new Date()
            property date selectedDate: systemClock ? systemClock.date : new Date()
            ...
            Repeater {
                ...
                Rectangle {
                    readonly property date dayDate: {
                        const d = new Date(parent.firstDay)
                        if (isNaN(d.getTime())) return new Date()
                        d.setDate(d.getDate() + index)
                        return d
                    }
                    readonly property bool isCurrentMonth: dayDate && !isNaN(dayDate.getTime()) && calendarGrid.displayDate && !isNaN(calendarGrid.displayDate.getTime()) ? dayDate.getMonth() === calendarGrid.displayDate.getMonth() : false
                    readonly property bool isToday: dayDate && !isNaN(dayDate.getTime()) ? dayDate.toDateString() === new Date().toDateString() : false
                    readonly property bool isSelected: dayDate && !isNaN(dayDate.getTime()) && calendarGrid.selectedDate && !isNaN(calendarGrid.selectedDate.getTime()) ? dayDate.toDateString() === calendarGrid.selectedDate.toDateString() : false
