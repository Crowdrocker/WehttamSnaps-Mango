...
Column {
    ...
    // Match Dock scaling: global UI scale × dock scale.
                ...
                Repeater {
                    ...
                    Item {
                        ...
                        height: {
                            ...
                            if (id === "volumeMixer") {
                                // Let the component report its true height.
                                // This prevents the background ending early while stream cards render below.
                                if (widgetLoader.item && widgetLoader.item.implicitHeight > 0)
                                    return widgetLoader.item.implicitHeight

                                // Fallback estimate before the loader instantiates.
                                ...
                                return root.spx(44) + Theme.spacingXS + shown * root.spx(40)
                                    + Math.max(0, shown - 1) * Theme.spacingS
                                    + (oc + ic > 3 ? (Theme.fontSizeSmall + root.spx(6)) : 0)
                                    + Theme.spacingS
                            }
                            if (id === "worldClocks") {
                                if (widgetLoader.item && widgetLoader.item.implicitHeight > 0)
    ...
    Component {
        ...
        // Resolve `CompoundPill` from the dock module import above.
        CompoundPill {
