...
Item {
    ...
    // Screen assignment ID matching DisplaysTab.variantComponents
    property string componentId: "miniPanelAppDrawer"

    // ── Scaling ───────────────────────────────────────────────────────────────
    readonly property real ui:    (Appearance.combinedScale || 1)
    readonly property real barUi: ui * (SettingsData.miniPanelScale || 1.0)
    ...
    readonly property real topBarThickness: {
        ...
        if (SettingsData.topBarVisible && SettingsData.topBarPosition === "top")
            t += ((SettingsData.topBarHeight || 40)
                  * (SettingsData.topbarScale || 1)
                  + (SettingsData.topBarSpacing || 4)
                  + (SettingsData.topBarTopMargin || 0)) * s
        if (SettingsData.showMiniPanel && SettingsData.minipanelPosition === "top"
            ...
            t += ((SettingsData.miniPanelHeight || 48)
                  ...
                  + (SettingsData.miniPanelBottomGap || 0)) * barUi
        return t
    }
    readonly property real bottomBarThickness: {
        if (SettingsData.showminipanel && !SettingsData.minipanelFloating)
            return (((SettingsData.minipanelExclusiveZone || SettingsData.minipanelHeight || 64)
                     * (SettingsData.minipanelScale || 1))
                    + (SettingsData.minipanelBottomGap || 0)) * s
        if (SettingsData.minipanelVisible && !SettingsData.minipanelFloat)
            return ((SettingsData.minipanelHeight || 54) * (SettingsData.minipanelScale || 1)
                    + (SettingsData.topBarSpacing || 4)) * s
        return 0
    }
    ...
    MiniPanelAppDrawer.AppLauncher {
        id: appLauncher
    ...
    PanelWindow {
        ...
        function updatePosition() {
            ...
            if (section === "minipanel") section = appDrawerPopout.barPosition
            ...
            let localX = targetX
            let localY = targetY
            if (appDrawerPopout.triggerX >= (screen.x || 0)) localX -= (screen.x || 0)
            if (appDrawerPopout.triggerY >= (screen.y || 0)) localY -= (screen.y || 0)
                ...
                Rectangle {
                    ...
                    Column {
                        ...
                        AppMenuEntry {
                            ...
                            label:    contextMenu.isPinned ? "Unpin from minipanel" : "Pin to minipanel"
                            onActivated: {
