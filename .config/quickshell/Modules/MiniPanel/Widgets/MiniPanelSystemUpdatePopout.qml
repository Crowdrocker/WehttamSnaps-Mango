...
/**
 * minipanelSystemUpdatePopout - A system update popup for the minipanel
 * Uses the same PanelWindow positioning pattern as minipanelVolumeMixerPopout
 */
Item {
    id: root
    ...
    function open() {
        // Use stored bar positioning properties
        openForItem(null, null, root.barPosition, false, root.barThickness)
    }
    ...
    function toggle() {
        if (currentPopup && currentPopup.showPopup) {
            close()
        } else {
            open()
        }
    ...
    // Bar positioning properties (set by widget before toggle)
    ...
    Component {
        ...
        Item {
            ...
            function showForAnchor(anchor, screen, barPos, vertical, barThick) {
                anchorItem = anchor
                parentScreen = screen || root.storedTriggerScreen
                barPosition = barPos || root.barPosition || "top"
                isVertical = vertical || false
                barThickness = barThick || root.barThickness || 48
            ...
            PanelWindow {
                ...
                function updatePosition() {
                    ...
                    if (!popupRoot.anchorItem) {
                        ...
                        const storedX = root.storedTriggerX
                        const storedY = root.storedTriggerY
                        
                        if (popupRoot.barPosition === "top") {
                            targetX = storedX + root.storedTriggerWidth / 2
                            targetY = screenY + scaledBarThickness + popupRoot.popupDistance + (15 * scale)
                        } else if (popupRoot.barPosition === "bottom") {
                            targetX = storedX + root.storedTriggerWidth / 2
                            targetY = screenY + screen.height - scaledBarThickness - popupRoot.popupDistance - (15 * scale)
                        } else if (popupRoot.barPosition === "left") {
                            targetX = screenX + scaledBarThickness + popupRoot.popupDistance + (15 * scale)
                            targetY = storedY
                        } else if (popupRoot.barPosition === "right") {
                            targetX = screenX + screen.width - scaledBarThickness - popupRoot.popupDistance - (15 * scale)
                            targetY = storedY
                        } else {
                            targetX = storedX + root.storedTriggerWidth / 2
                            targetY = storedY
