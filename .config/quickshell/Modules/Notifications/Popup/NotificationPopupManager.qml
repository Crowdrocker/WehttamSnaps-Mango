...
QtObject {
    ...
    property int baseNotificationHeight: {
        // Treat window as stable surface; stack popups using the
        // configured maximum card height + a configurable gap.
        const maxH = (typeof SettingsData !== "undefined" && SettingsData.notificationPopupMaxHeight !== undefined)
            ? SettingsData.notificationPopupMaxHeight : 260
        const gap = (typeof SettingsData !== "undefined" && SettingsData.notificationPopupStackSpacing !== undefined)
            ? SettingsData.notificationPopupStackSpacing : 10
        const margin = (typeof SettingsData !== "undefined" && SettingsData.notificationPopupMargin !== undefined)
            ? SettingsData.notificationPopupMargin : 6
        return Math.max(80, maxH + gap + margin * 2)
    }
    ...
    function _refreshPositions() {
        const active = _active().sort((a, b) => {
            const seqA = a.notificationData ? a.notificationData.seq || 0 : 0
            const seqB = b.notificationData ? b.notificationData.seq || 0 : 0
            return seqB - seqA
        })
        for (let k = 0; k < active.length; ++k) {
            active[k].screenY = topMargin + k * baseNotificationHeight
        }
    }
