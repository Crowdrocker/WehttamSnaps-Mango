...
Item {
    ...
    // Thickness of the triggering bar (TaskBar) in screen px.
    property real   barThickness:   48 * (Appearance.combinedScale || 1) * (SettingsData.taskbarScale || 1)
    ...
    readonly property real barUi:   ui * (SettingsData.taskbarScale || 1)
    ...
    readonly property real bottomBarThickness: {
        ...
        if (!SettingsData.taskBarVisible) return 0

        // If the TaskBar is floating, it doesn't reserve an exclusive zone, but it
        // does still occupy space (its own height) and sits above the bottom edge
        // by `taskBarBottomMargin`. We must treat BOTH as "reserved space" when
        // positioning popouts, otherwise rounded corners/shadows can get clipped.
        if (SettingsData.taskBarFloat) {
            const h = (SettingsData.taskBarHeight || SettingsData.taskBarExclusiveZone || 64)
            const m = (SettingsData.taskBarBottomMargin || 0)
            return (h + m) * barUi
        }

        return (SettingsData.taskBarExclusiveZone || SettingsData.taskBarHeight || 64) * barUi
    }
    ...
    PanelWindow {
        ...
        function updatePosition() {
            ...
            if (section === "taskbar") section = appDrawerPopout.barPosition
            ...
            const maxY = screen.height - bottomBarSpace - popupH - spacing - extraPadding
            ...
            Item {
                ...
                Row {
                    ...
                    Rectangle {
                        ...
                        ColumnLayout {
                            anchors.fill:    parent
                            anchors.margins: Theme.spacingL * launcherPanel.s
                            spacing:         Theme.spacingL * launcherPanel.s
                            clip:            true
                            ...
                            ColumnLayout {
                                ...
                                spacing: Theme.spacingM * launcherPanel.s
                                clip: true
                                ...
                                Grid {
                                    Layout.fillWidth: true
                                    columns:      3
                                    rowSpacing:   (Theme.spacingM * launcherPanel.s) / 2
                                    columnSpacing:(Theme.spacingM * launcherPanel.s) / 2
                                    clip: true
                ...
                Rectangle {
                    ...
                    Column {
                        ...
                        AppMenuEntry {
                            ...
                            label:    contextMenu.isPinned ? "Unpin from taskbar" : "Pin to taskbar"
                            onActivated: {
