...
PanelWindow {
    ...
    Rectangle {
        ...
        property real textWidth: {
            var msg = ToastService.currentMessage
            var fontSize = Theme.fontSizeMedium
            var metrics = Qt.createQmlObject('import QtQuick; TextMetrics { text: "' + msg + '"; font.family: "Inter"; font.pixelSize: ' + fontSize + ' }', toast)
            var w = metrics.width
            metrics.destroy()
            return w
        }
        ...
        readonly property real minWidthWithDetails: 380
        readonly property real minWidthWithoutDetails: 350
        
        // Calculate required width (text + icon + margins + spacing)
        property real requiredWidth: {
            var baseWidth = textWidth + Theme.iconSize + Theme.spacingL * 3 + Theme.spacingM * 2
            // Add space for expand/close buttons if visible
            if (ToastService.hasDetails) {
                baseWidth += (Theme.iconSize + 8) * 2 + Theme.spacingM
            }
            return baseWidth
        }
        ...
        width: shouldBeVisible ? Math.max(ToastService.hasDetails ? minWidthWithDetails : minWidthWithoutDetails, requiredWidth) : frozenWidth
        height: toastContent.height + Theme.spacingL * 2
        anchors.horizontalCenter: parent.horizontalCenter
        y: (SettingsData.topBarHeight * SettingsData.topbarScale) - 4 + SettingsData.topBarSpacing + 2
        color: {
