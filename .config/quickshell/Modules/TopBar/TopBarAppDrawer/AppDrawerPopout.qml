...
Item {
    ...
    property real   barThickness:   48 * barUi

    property bool shouldBeVisible: false

    // ── Scaling ───────────────────────────────────────────────────────────────
    readonly property real ui:    (Appearance.combinedScale || 1)
    readonly property real barUi: ui * (SettingsData.topbarScale || 1)
    ...
    readonly property real popupWidth: {
        const scr = triggerScreen || (Quickshell.screens && Quickshell.screens.length ? Quickshell.screens[0] : null)
        const desired = 780 * s
        if (!scr) return desired
        return Math.min(desired, scr.width - Theme.spacingL * 2)
    }
    readonly property real popupHeight: {
        const scr = triggerScreen || (Quickshell.screens && Quickshell.screens.length ? Quickshell.screens[0] : null)
        const desired = 720 * s
        if (!scr) return desired
        const topBarSpace = topBarThickness
        const bottomBarSpace = bottomBarThickness
        const extraPadding = Theme.spacingS
        const spacing = Theme.spacingM
        const availH = scr.height - topBarSpace - bottomBarSpace - Theme.spacingM * 2 - extraPadding * 2
        return Math.min(desired, Math.max(200, availH - spacing * 2))
    }
    ...
    readonly property real topBarThickness: {
        ...
        if (SettingsData.topBarVisible && SettingsData.topBarPosition === "top")
            t += ((SettingsData.topBarHeight || 40)
                  + (SettingsData.topBarSpacing || 4)
                  + (SettingsData.topBarTopMargin || 0)) * barUi
        // MiniPanel has no "float" setting; only include when actually shown on top.
        if (SettingsData.showMiniPanel && SettingsData.minipanelPosition === "top")
            t += ((SettingsData.miniPanelHeight || 48)
                  + (SettingsData.miniPanelSpacing || 4)
                  + (SettingsData.miniPanelBottomGap || 0)) * ui * (SettingsData.miniPanelScale || 1.0)
        return t
    }
    readonly property real bottomBarThickness: {
        // TaskBar uses camelCase settings.
        if (SettingsData.taskBarVisible && !SettingsData.taskBarFloat)
            return (SettingsData.taskBarExclusiveZone || SettingsData.taskBarHeight || 64)
                   * ui * (SettingsData.taskbarScale || 1)
        return 0
    }
    ...
    PanelWindow {
        ...
        function updatePosition() {
            ...
            // Use actual barThickness provided by launcher button for tight alignment.
            const topBarSpace   = (SettingsData.topBarTopMargin || 0) * appDrawerPopout.s
                                + (appDrawerPopout.barThickness || 0) * appDrawerPopout.s
            const bottomBarSpace = appDrawerPopout.bottomBarThickness
            // Keep the drawer visually tight to the bar edge.
            // Use a fixed 2px gap (scaled only by global UI), not Theme spacing.
            const gapPx         = Math.round(2 * appDrawerPopout.ui)
            const extraPadding  = gapPx
            const spacing       = gapPx
            const flushGap      = gapPx

            const availH = screen.height - topBarSpace - bottomBarSpace
                           - Theme.spacingM * 2 - extraPadding * 2

            const popupW = appDrawerPopout.popupWidth
            const popupH = appDrawerPopout.popupHeight

            let targetX = 0, targetY = 0
            let section = appDrawerPopout.triggerSection
            if (section === "") section = appDrawerPopout.barPosition

            const cx = appDrawerPopout.triggerX + appDrawerPopout.triggerWidth / 2

            if (section === "bottom") {
                targetX = cx - popupW / 2
                targetY = screen.height - bottomBarSpace - popupH - flushGap
            } else if (section === "top") {
                targetX = cx - popupW / 2
                // Align panel tightly under top bar
                targetY = topBarSpace + flushGap
            } else if (section === "left") {
                targetX = appDrawerPopout.triggerX + appDrawerPopout.triggerWidth + flushGap
                targetY = topBarSpace + flushGap + (availH - popupH) / 2
            } else if (section === "right") {
                targetX = appDrawerPopout.triggerX - popupW - flushGap
                targetY = topBarSpace + flushGap + (availH - popupH) / 2
            } else {
            ...
            // Convert global coords to screen-local coords (always subtract screen origin).
            let localX = targetX - (screen.x || 0)
            let localY = targetY - (screen.y || 0)

            const minY = (section === "top")
                ? (topBarSpace + flushGap)
                : (topBarSpace + flushGap)
            // Max Y should keep panel fully on-screen above bottom bar.
            const maxY = screen.height - bottomBarSpace - popupH - flushGap

            localX = Math.max(flushGap, Math.min(localX, screen.width - popupW - flushGap))
                ...
                Rectangle {
                    ...
                    Column {
                        ...
                        AppMenuEntry {
                            ...
                            label:    contextMenu.isPinned ? "Unpin from " : "Pin to "
                            onActivated: {
