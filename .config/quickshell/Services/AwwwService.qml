...
Singleton {
    ...
    Process {
        ...
        onExited: exitCode => {
            if (exitCode !== 0) {
                ...
                // Wallpaper was set successfully - update SessionData and trigger dynamic theming
                if (typeof SessionData !== 'undefined' && imagePath) {
                    SessionData.setWallpaper(imagePath)
                }
                // Trigger real-time dynamic theming
                if (typeof ColorPaletteService !== 'undefined' && imagePath) {
                    ColorPaletteService.onWallpaperChanged(imagePath)
                }
            }
        }
        property string imagePath: ""  // Store the image path for onExited callback
    ...
    function setWallpaper(screenName, imagePath) {
        ...
        wallpaperSetProcess.imagePath = imagePath  // Store for onExited callback
