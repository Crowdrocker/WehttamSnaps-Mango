...
Singleton {
    ...
    // "devices" contains controllable brightness endpoints (backlight + DDC).
    // Excludes e.g. keyboard / NIC LEDs, so UI doesn't show bogus sliders.
    ...
    function refreshDevicesInternal() {
        ...
        if (devices.length > 0 && !currentDevice) {
            ...
            if (deviceExists) {
                ...
                // Prefer actual backlight device if present, else first available.
                const preferred = devices.find(d => d.class === "backlight") || devices[0]
                setCurrentDevice(preferred.name, false)
            }
        }
    }
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                for (const line of lines) {
                    ...
                    if (parts.length >= 5) {
                        const devClass = parts[1]
                        // Ignore non-brightness devices (keyboard LEDs, NIC LEDs, etc.)
                        if (devClass === "leds")
                            continue
