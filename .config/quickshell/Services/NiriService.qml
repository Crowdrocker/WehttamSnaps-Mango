...
Singleton {
    ...
    function handleScreenshotCaptured(data) {
        ...
        if (editor && editor.length) {
            const cmd = editor.split(" ").filter(p => p.length).map(p => p === "%path%" ? data.path : p)
            Quickshell.execDetached(cmd)
        } else {
            Quickshell.execDetached(["sh", "-c",
                `command -v satty >/dev/null 2>&1 && exec satty -f "${data.path}"; ` +
                `command -v swappy >/dev/null 2>&1 && exec swappy -f "${data.path}"; ` +
                `exit 0`
            ])
        }

        if (typeof ToastService !== "undefined")
            ToastService.showInfo("Screenshot saved", data.path)
    ...
    function screenshot(path) {
        pendingScreenshotPath = ""
        const outPath = path && path.length ? path : `${screenshotsDir}/screenshot-${Date.now()}.png`
        pendingScreenshotPath = outPath
    ...
    function screenshotScreen(path) {
        pendingScreenshotPath = ""
        const outPath = path && path.length ? path : `${screenshotsDir}/screenshot-${Date.now()}.png`
        pendingScreenshotPath = outPath
        Quickshell.execDetached(["sh", "-c", `mkdir -p "${screenshotsDir}"`])
    ...
    function screenshotWindow(path) {
        pendingScreenshotPath = ""
        const outPath = path && path.length ? path : `${screenshotsDir}/screenshot-${Date.now()}.png`
        pendingScreenshotPath = outPath
        Quickshell.execDetached(["sh", "-c", `mkdir -p "${screenshotsDir}"`])
    ...
    function moveWindowToWorkspace(workspaceIndex) {
        return send({
                        "Action": {
    ...
    function getOutputScale(outputName) {
        if (!outputs || !outputName) return 1
        const output = outputs[outputName]
        if (output && output.scale) return output.scale
        return 1
    }
