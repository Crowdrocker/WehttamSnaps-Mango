...
Singleton {
    ...
    function showToast(message, level = levelInfo, details = "") {
        ...
        const levelName = level === levelError ? "ERROR" : (level === levelWarn ? "WARN" : "INFO")
        const logMsg = details ? `${message} | Details: ${details}` : message
        LoggingService.log(LoggingService.logInfo, "Toast", `[${levelName}] ${logMsg}`)
    ...
    function hideToast() {
        // Log toast hidden
        LoggingService.log(LoggingService.logDebug, "Toast", `Hidden toast: ${currentMessage}`)
